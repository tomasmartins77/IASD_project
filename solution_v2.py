...
class FleetProblem(Problem):
    ...
    def load(self, file):
        ...
        self.requests.sort(key=lambda x: x.get_time())
        self.fleet.sort(key=lambda x: x.get_capacity(), reverse=False)
    ...
    def actions(self, state):
        ...
        for vehicle in vehicles:
            for request in requests:
                if request.get_status() == 'waiting' and vehicle.get_capacity() >= request.get_passengers():
                    ...
                    # print('Action: Pickup ', 'vehicle: ', vehicles.index(vehicle), 'request: ', requests.index(request), 'pickup_time: ', pickup_time)
                ...
                if request.get_status() == 'traveling' and request.get_vehicle_id() == vehicles.index(vehicle):
                    ...
                    # print('Action: Dropoff ', 'vehicle: ', vehicles.index(vehicle), 'request: ', requests.index(request), 'dropoff_time: ', dropoff_time)
        ...
        #             if vehicle not in test:
        #                 test[vehicle] = [requests.get_pickup(), requests.get_passengers()]
        #                 actions.append(Action('Pickup', vehicles.index(vehicle), requests.index(request), pickup_time))
        #                 break
        #             elif test[vehicle][0] != requests.get_pickup():
        #                 test = dict()
    ...
    def result(self, state, action):
        ...
        if type == 'Pickup':
            request.pick_request(action.get_vehicle_id())
    ...
    def path_cost(self, c, state1, action, state2):
        ...
        if action.get_type() == 'Pickup':
            ...
            if request.get_dropoff() not in locations and len(locations) != 0:
                beta = 1.2
            c += (time - request.get_time() - self.graph.get_edge(vehicle.get_location(), request.get_dropoff())) * beta
...
class Request:
    ...
    def pick_request(self, vehicle_id):
        """Picks up a request."""
...
class State:
    ...
    def __init__(self, requests, vehicles, time = 0):
        self.requests = requests
